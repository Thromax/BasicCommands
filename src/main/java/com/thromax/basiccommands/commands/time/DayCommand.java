...
import static com.thromax.basiccommands.constants.Permissions.PERMISSION_CMD_TIME_DAY;

/**
 * It turns day in the selected world.
 * <p><br><b>Usage: </b>/day [world name or index]</p>
 */
public class DayCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            if (sender.hasPermission(PERMISSION_CMD_TIME_DAY)) {
                World w;
                if (sender instanceof Player) {
                    w = ((Player) sender).getWorld();
                    sender.sendMessage(configStringParser(CONFIG_DAY_CURRENT_WORLD_MESSAGE, sender.getName(), w.getName()));
                } else {
                    w = sender.getServer().getWorlds().get(0);
                    sender.sendMessage(configStringParser(CONFIG_DAY_ON_WORLD_MESSAGE, sender.getName(), w.getName()));
                }
                w.setTime(DAY_TIME);
            } else
